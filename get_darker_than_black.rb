...
class GetDarkerThanBlack
  ...
  def prep_chapters
    ...
    page = @agent.get("http://www.mangareader.net/716/darker-than-black-shikkoku-no-hana.html")
    @chapter_markers = {}
    i = 1
    @first_page = 0
    load_chapter = 1
  ...
  def start_download
    cbz_dir = "darkerthanblack_comics"
  ...
  def chapter_directory(chapter_id)
    "darkerthanblack_comics/darkerthanblack_chapter_#{chapter_id}"
  end
  ...
  def zip_file_path(chapter_id)
    ...
    "#{directory_name}/darkerthanblack_chapter_#{chapter_id}.cbz"
  end
...
GetDarkerThanBlack.new_download(nil)
